                ...
                <table class="table table-striped">
                    ...
                    @forelse($albums as $album)
                        <tr>
                            ...
                            <td>{{$album->name}}</td>
                            <td>{{\Carbon\Carbon::parse($album->release)->isoFormat('D MMM Y')}}</td>
                            <td>{{$album->artist->name}}</td>
                            <td>@forelse($album->images as $image)
                            ...
                            <td>
                                <div class="form-group">
                                    <a href="" class="btn btn-outline-warning" data-toggle="modal" data-target="#exampleModal-{{$album->id}}"
                                       id="#myBtn" ><i class="feather icon-edit-2"></i></a>
{{--                                    <a href="" class="btn btn-outline-secondary" data-toggle="modal" data-target="#showModal-{{$album->id}}"--}}
{{--                                       id="#myBtn" ><i class="feather icon-eye"></i></a>--}}
                                    <form action="{{route('album.destroy',$album)}}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
...
{{--modal input--}}
    <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="form-group col-md-4">
                            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" placeholder="Masukkan Nama Album">
                            @error('name')
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <input type="date" name="release" class="form-control @error('release') is-invalid @enderror">
                            @error('release')
                            <span class="invalid-feedback text-capitalize">{{$message}}</span>
                            @enderror
                        </div>
        ...
        @include('back.albums.edit')
{{--        @include('backend.albums.show')--}}
@endsection
